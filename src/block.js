// Parte do codigo feito pela Alana
...
const checkCollision = (block, line) => {
  // 0 goon 1 drop 2 girar para a esquerda 3 girar para a direita 4 ok 5 perfeito
...
const checkBlockOut = (instance, engine) => {
  if (instance.status === constant.rotateLeft) {
    // Vire à esquerda Espere até que o canto superior direito desapareça para ser considerado desaparecido
...
export const blockAction = (instance, engine, time) => {
  ...
  switch (i.status) {
    ...
    case constant.beforeDrop:
      ...
      i.y = instance.weightY + (0.3 * instance.height) // adicionar altura da corda
      i.rotate = 0
      i.ay = engine.pixelsPerFrame(0.0003 * engine.height) // aceleração da gravidade
    ...
    case constant.drop:
      ...
      switch (collision) {
        ...
        case 5:
          ...
          // Detecção de trapaças 1/3 além da esquerda ou direita
    ...
    case constant.rotateRight:
      ...
      const isShouldFall = isRight ? instance.rotate > 1.3 : instance.rotate < -1.3// 75 graus
